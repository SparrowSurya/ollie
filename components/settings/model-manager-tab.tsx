...
export default function ModelManagerTab() {
  ...
  const renderCloudProviderSection = (provider: "openai" | "anthropic" | "gemini", label: string) => {
    const providerPrefix = `${provider}/`;
    const providerModels = runnableModels.filter((m) => m.startsWith(providerPrefix));
    const isKeyMissing = providerModels.length === 0 ? true : providerModels.every((m) => disabledModels.includes(m));
    const envKeyName = provider === "openai" ? "OPENAI_API_KEY" : provider === "anthropic" ? "ANTHROPIC_API_KEY" : "GEMINI_API_KEY";

    return (
      <div className="p-3.5 bg-base-content/5 border border-base-content/5 rounded-2xl flex flex-col gap-2.5 min-w-0">
        {/* Header */}
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 min-w-0">
            <span className="font-bold text-xs uppercase tracking-wider text-base-content shrink-0">{label}</span>
            {isKeyMissing ? (
              <span className="badge badge-warning text-[9px] font-bold py-0.5 h-4 border-none bg-warning/15 text-warning rounded-md select-none shrink-0">OFFLINE</span>
            ) : (
              <span className="badge badge-success text-[9px] font-bold py-0.5 h-4 border-none bg-success/15 text-success rounded-md select-none shrink-0">ONLINE</span>
            )}
          </div>
          <span className="text-[9px] font-mono text-base-content/40 truncate select-all">{envKeyName}</span>
        </div>

        {/* Warning Banner / Models */}
        {isKeyMissing ? (
          <div className="text-[11px] text-warning bg-warning/10 border-none p-2.5 rounded-xl leading-normal font-sans text-left">
            API key is not configured on the server. Models from {label} will remain disabled.
          </div>
        ) : (
          <div className="flex flex-col gap-1.5 min-w-0 text-left">
            <span className="text-[9px] font-bold uppercase tracking-wider text-base-content/50 shrink-0">Available Models:</span>
            <div className="flex flex-wrap gap-1.5">
              {providerModels.map((m) => {
                ...
                return (
                  <span
                    key={m}
                    className={`badge text-[9.5px] font-mono py-1 h-5 rounded-md border border-base-content/10 truncate max-w-full ${
                      isDisabled ? "bg-base-content/5 text-base-content/30 line-through" : "bg-user-accent/15 text-user-accent font-bold border-user-accent/35"
                    }`}
                  >
                    {displayName}
                  </span>
    ...
    <div className="flex flex-col h-full overflow-hidden select-none">
      ...
      <div className="flex-1 overflow-y-auto pr-1">
        <div className="flex flex-col gap-1 pb-4">
                        ...
                        <div className="flex items-center gap-2">
                          {pullState ? (
                            ...
                            <button
                              ...
                              className="btn btn-xs shrink-0 border-user-accent bg-user-accent/10 hover:bg-user-accent hover:border-user-accent text-user-accent hover:text-base-100 uppercase tracking-wider font-bold h-6 text-[10px] px-3.5 rounded-full cursor-pointer"
                            >
                              Pull
                            </button>
                          )}
          ...
          {/* External Cloud Models Section */}
          <div className="flex flex-col gap-3 py-4 border-t border-base-content/5 mt-4 select-none text-left">
            <div className="flex flex-col gap-0.5">
              <span className="text-base font-bold uppercase tracking-wider text-base-content">
                External Cloud Models:
              </span>
              <span className="text-xs text-base-content/60 leading-relaxed font-sans mb-1">
                Configure remote model providers via server environment variables.
              </span>
            </div>

            <div className="flex flex-col gap-3.5 mt-1.5">
              {renderCloudProviderSection("gemini", "Google Gemini")}
              {renderCloudProviderSection("openai", "OpenAI")}
              {renderCloudProviderSection("anthropic", "Anthropic")}
            </div>
          </div>
        </div>
      </div>
